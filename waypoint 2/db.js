...
/* Default display order (used front-of-house AND in admin):
   ...
   2) then the manual sort_order (ascending)
   3) then rating desc, then review count desc  */
...
export async function listHotels({ city } = {}) {
  ...
  // SQL mirror of defaultSort
  const orderSql = `ORDER BY
      (CASE WHEN COALESCE(NULLIF(TRIM(seo),''),'') = '' THEN 1 ELSE 0 END),
      sort_order ASC, rating DESC NULLS LAST, review_count DESC NULLS LAST`;
  const q = city
