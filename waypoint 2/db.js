...
async function upsertHotelClient(client, h) {
  await client.query(
    `INSERT INTO hotels
       (id,city,city_name,name,place,rating,review_count,rank,price,tags,gradient,tc,lat,lng,seo,about,amenities,sub_ratings)
...
export async function listHotels({ city } = {}) {
  ...
  const q = city
    ? await pool.query("SELECT * FROM hotels WHERE city=$1", [city])
    : await pool.query("SELECT * FROM hotels");
