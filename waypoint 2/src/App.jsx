  ...
  return (
    <div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, overflow: "hidden", marginBottom: 24 }}>
        {listing.city
          ? <CityArt gradient={listing.gradient} big flat />
...
function CityArt({ gradient, big, flat }) {
...
function HomePage({ onOpenCity, onOpenListing, votes, cities, hotels }) {
  const [q, setQ] = useState("");

  const countFor = (key) => hotels.filter(l => l.city === key).length;
  // only show cities that actually have hotels
  const shownCities = cities
    .filter(c => countFor(c.key) > 0)
    .filter(c => (c.name + c.country).toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <div>
      <div style={{ textAlign: "center", padding: "38px 16px 26px" }}>
        <h1 style={{ fontFamily: "'Poppins', sans-serif", fontSize: "clamp(30px, 5vw, 46px)", fontWeight: 700, color: C.ink, margin: "0 0 22px", lineHeight: 1.08 }}>
        ...
        <div style={{ maxWidth: 540, margin: "0 auto", display: "flex", border: `1.5px solid ${C.ink}`, borderRadius: 99, overflow: "hidden", background: C.card }}>
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search a city — try “Tokyo”, “Rome”, “Paris”…"
            style={{ flex: 1, border: "none", outline: "none", padding: "13px 20px", fontSize: 15, fontFamily: "'Roboto', sans-serif", background: "transparent", color: C.ink }} />
          ...
          <div key={c.key} onClick={() => onOpenCity(c.key)} style={{
            ...
            <div style={{ padding: "14px 16px 16px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontFamily: "'Roboto Mono', monospace", fontSize: 11.5, color: C.buoy, letterSpacing: "0.04em" }}>
                  {countFor(c.key)} hotels
                </span>
      ...
      {shownCities.length === 0 && (
        <div style={{ textAlign: "center", color: C.inkSoft, padding: 40, fontSize: 14.5 }}>
          No city matches “{q}”. We currently cover {cities.filter(c => countFor(c.key) > 0).map(c => c.name).join(", ")}.
        </div>
      )}
...
function CityPage({ cityKey, onBack, onOpen, votes, cities, hotels: allHotels }) {
  ...
  const hotels = useMemo(() => {
    ...
    return list;
  }, [cityKey, sort, onlyTC, q, allHotels]);

  const tcCount = allHotels.filter(l => l.city === cityKey && l.tc).length;
  ...
  // reset to page 1 whenever the filter/sort/city changes
  useEffect(() => { setPage(1); }, [cityKey, sort, onlyTC, q]);
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 22 }}>
        <CityArt gradient={city.gradient} big />
        <div style={{ marginTop: 16 }}>
          ...
          <h1 style={{ fontFamily: "'Poppins', sans-serif", fontSize: "clamp(28px, 4vw, 40px)", fontWeight: 700, color: C.ink, margin: "0 0 6px", lineHeight: 1.08 }}>
            Best hotels in {city.name}
          </h1>
          <p style={{ fontSize: 14.5, color: C.inkSoft, margin: 0 }}>
            {hotels.length} places to stay · {tcCount} Travellers' Choice winners · hotels with an AI review shown first.
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="Filter by name or amenity…"
          style={{ flex: "1 1 200px", minWidth: 160, border: `1px solid ${C.line}`, borderRadius: 8, padding: "8px 12px", fontSize: 13.5, fontFamily: "'Roboto', sans-serif", background: C.paper, color: C.ink, outline: "none" }} />
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {pageHotels.map((l, i) => (
          <CityHotelRow key={l.id} l={l} rank={(curPage - 1) * PER_PAGE + i + 1} onOpen={() => onOpen(l)} votes={votes} />
        ))}
      </div>
      {hotels.length === 0 && (
        <div style={{ textAlign: "center", color: C.inkSoft, padding: 40, fontSize: 14.5 }}>
...
function CityHotelRow({ l, rank, onOpen, votes }) {
  ...
  useEffect(() => {
    // Only fetch a guest quote when there is no SEO summary to show.
    if (l.seo) return;
    let alive = true;
    fetchJson(`/api/hotels/${encodeURIComponent(l.id)}/reviews`)
      .then(rs => { if (alive) setQuote((rs || []).find(r => r.source === "quote") || null); })
      .catch(() => {});
    return () => { alive = false; };
  }, [l.id, l.seo]);

  const showSeo = Boolean(l.seo);
  const badge = showSeo ? "AI" : "GUEST";
  const badgeColor = showSeo ? C.buoy : C.green;
  const text = showSeo ? l.seo : quote?.text;
    ...
    <div onClick={onOpen} style={{
      ...
      <div style={{ padding: "14px 18px", minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
          <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 18, fontWeight: 700, color: C.inkSoft }}>{rank}.</span>
          <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 18.5, fontWeight: 700, color: C.ink, lineHeight: 1.2 }}>{l.name}</span>
        </div>
        ...
        {text && (
          <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
            ...
            <p style={{ fontSize: 13, lineHeight: 1.55, color: C.inkSoft, margin: 0 }}>
              {showSeo ? text : `“${text}”`}
              {!showSeo && quote?.author && <span style={{ color: C.inkSoft, fontStyle: "italic" }}> — {quote.author}</span>}
            </p>
          </div>
        )}
        {votes && (
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.paper, fontFamily: "'Roboto', sans-serif", color: C.ink }}>
      ...
      <footer style={{ borderTop: `1px solid ${C.line}`, padding: "22px 20px", textAlign: "center" }}>
        <div style={{ fontFamily: "'Roboto Mono', monospace", fontSize: 11, color: C.inkSoft, letterSpacing: "0.08em" }}>
          FIND A HOTEL · demo prototype · hotel data from Travellers' Choice · quotes are real guest excerpts
        </div>
      </footer>
    </div>
  );
